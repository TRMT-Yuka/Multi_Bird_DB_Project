...
このときの checkpoint は `data/interim/wikidata/local_dump_extract_checkpoint.json` に保存されます。`data/` 配下は `.gitignore` で除外しているので、これは各利用者のローカル状態として扱われ、OSS 利用者の Git 履歴には含めません。
